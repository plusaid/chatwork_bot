...
function doPost(e) {
    try {
        ...
        // 無限ループを回避する為の対処
        var roopBlock = webhookJson.webhook_event.body.match(/プロフ設定と自己紹介が終わってないと発言禁止〜/);
        if (!roopBlock) {
            //======ホワイトリスト検索=====
            var Range="B1:B"+ whitelist.getRange("B:B").getLastRow();
            var checkSelfIntro = whitelist.getRange(Range).createTextFinder(webhookJson.webhook_event.account_id).matchEntireCell(true).findNext();
            ...
            if (checkSelfIntro == null || getChatworkMessageJson.account.avatar_image_url.match(defaultProfielImgRegex) || webhookJson.webhook_event.body == 'chatwork_bot_debug') {
                var title = "プロフ設定と自己紹介が終わってないと発言禁止〜";
